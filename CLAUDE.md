...
> **요청한 것만 수정한다. 요청하지 않은 것은 절대 변경하지 않는다.**

- 사용자가 "A를 고쳐달라"고 하면 **A만** 수정한다. B, C는 건드리지 않는다.
- 디자인 에셋(이미지, 아이콘, 색상 등)을 교체·재생성할 때는 **반드시 사용자에게 먼저 확인**한다.
- 수정 범위가 불명확하면 **구현 전에 질문**한다. 추측해서 더 많이 고치지 않는다.
- 빌드·설치 후 **스크린샷으로 직접 결과를 확인**하고, 문제가 있으면 사용자에게 보고하기 전에 먼저 수정한다.
